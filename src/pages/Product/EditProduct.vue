<template>
  <div>
            ...
            <form action="/abc" ref="productForm" @submit.prevent="submitForm">
              ...
              <ul class="product-info-list">
                <li class="product-info-item">
                  ...
                  <input type="text" name="pid" v-model="param" disabled class="product-info-input" placeholder="商品编号" />
          ...
          <div class="variants">
            <transition-group name="slide">
              <template v-for="variant in variants" :key="variant">
                <ProductCard :variant="variant" @deleteVariant="deleteVariant" @updateVariant="updateVariant"
    ...
    <q-dialog v-model="deleteItemConfirm">
      <PromptBox @cancel="deleteItemConfirm = false" @deleteEvent="deleteEvent"></PromptBox>
    </q-dialog>
...
import ProductCard from "src/components/Common/VariantCard.vue";
...
const variants = ref([
  // {
  //   id: 1,
  //   image: "/images/products/Ragdolls/可爱柴犬布偶/可爱柴犬布偶.png",
  //   name: "玩具熊",
  //   price: 15.0,
  //   inventory: 0,
  //   style: "黑色",
...
watch(
  ...
  (newItems, oldItems) => {
    ...
    console.log(checkedVariant.value);
  },
  { deep: true }
);

const handleClickVariant = (variant) => {
  const clickedVariant = variants.value.find((item) => item.variant_id === variant.variant_id);
...
const fetchDataAndSetRows = async () => {
  try {
    ...
    variants.value = data.data.filter(item => item.product_id === +param.value)
  } catch (error) {
...
const deleteEvent = () => {
  if (deleteVariantID.value) {
    variants.value = variants.value.filter(
      (item) => item.id !== deleteVariantID.value
    );
  } else {
    variants.value = variants.value.filter((item) => item.checked !== true);
  }
  deleteItemConfirm.value = false;
  showNotif("positive", "删除成功", "check");
};
